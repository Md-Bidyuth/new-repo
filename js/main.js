...
const handleLoadCategory = async (categoryId) => {
  ...
  if (isSort === true) {
    data.data
      .sort((a, b) => {
        const viewsA = parseFloat(a.others.views.replace("K", "")) * 1000; // Convert 'K' to thousands
        const viewsB = parseFloat(b.others.views.replace("K", "")) * 1000; // Convert 'K' to thousands
        return viewsA - viewsB;
      })
      .forEach((single_category) => {
        const hours = Math.floor(single_category.others.posted_date / 3600);
        const minutes = (
          (single_category?.others?.posted_date / 3600 - hours) *
          60
        ).toFixed(0);
        const div = document.createElement("div");
        div.innerHTML = `<div class="card w-auto h-80 bg-base-100 shadow-xl p-2 relative">
            <figure class ="h-40 mb-8 "> <img src ="${
              single_category.thumbnail
            }"
            alt ="Item Photo"> </figure> 
            <p class="bg-[#171717] text-white absolute right-4 top-32 px-1 rounded">${
              single_category?.others?.posted_date
           ...
           <div class="flex gap-4">
            <div>
              <img class="w-10 h-10 rounded-full" src="${
                single_category.authors[0].profile_picture
              }" alt="Author Photo">
            </div>
            <div class="">
              ...
              <div class="flex items-center">
                ...
                <p class ="me-2"> ${
                  single_category.authors[0].verified
                    ? '<i class="fa-solid fa-circle-check text-[#2568EF] text-lg"></i>'
                    : ""
                } </p >
               </div>
